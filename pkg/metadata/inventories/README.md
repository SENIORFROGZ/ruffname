...
- `agent_metadata` - **dict of string to JSON type**:
  ...
  - `feature_security_enabled` - **string**: Either `"true"` or `"false"` depending if the Security Agent is enabled
    (see: `runtime_security_config.enabled` config option).
  - `feature_process_enabled` - **string**: Either `"true"` or `"false"` depending if the Process Agent is enabled (see:
    `process_config.enabled` config option).
  - `feature_networks_enabled` - **string**: Either `"true"` or `"false"` depending if the System Probe Agent is enabled
    (see: `system_probe_config.enabled` config option).
  - `feature_logs_enabled` - **string**: Either `"true"` or `"false"` depending if the logs collection is enabled (see:
    `logs_enabled` config option).
  - `feature_compliance_enabled` - **string**: Either `"true"` or `"false"` depending if the Cloud Security Posture
    Management is enabled (see: `compliance_config.enabled` config option).
  - `feature_apm_enabled` - **string**: Either `"true"` or `"false"` depending if the APM Agent is enabled (see:
    `apm_config.enabled` config option).
